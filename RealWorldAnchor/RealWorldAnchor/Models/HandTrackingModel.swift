...
class HandTrackingModel: TrackingModel, ObservableObject {
    let handDataProvider = HandTrackingProvider()
    
    @Published var testVariable = -1
        ...
        do {
            if HandTrackingProvider.isSupported {
                ...
                DispatchQueue.main.async {
                    ...
                    self.testVariable = 0
                    self.updateFingerEnity(idPrefix: "neutral", rootTransform: simd_float4x4(), skeleton: neutralPose)
                    self.testVariable = 1
                    self.testVariable = 2
                    self.testVariable = 3
                    print("Variable change end")
                }
